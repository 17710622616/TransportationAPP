...
public class printer {
    ...
    public void PrintMaterialListString(TestMaterialModel model, int textSize, int nLeftName, int nLeftNum, boolean bBold) {
        if (mLineBitmap != null) {
            mPaint.setTextSize(textSize);
            mPaint.setFakeBoldText(bBold);
            float y = Math.abs(mPaint.ascent());
            PrintLineOneString(model.getMaterialName(),nLeftName,y,mLineBitmap);
            PrintLineOneString(String.valueOf(model.getMaterialNum()),nLeftNum,y,mLineBitmap);
        }
    }
